...
import type SolarIndices from 'Frontend/generated/io/nextskip/propagation/model/SolarIndices.js';
import './SolarIndicesCard.css';
...
function SolarIndicesCard({ solarIndices }: Props) {
  const getSolarFluxLevel = (sfi: number): { label: string; className: string } => {
  ...
  const getGeomagneticLevel = (kIndex: number): { label: string; className: string } => {
  ...
  return (
    <div className="solar-indices-card card card-elevated">
      <div className="card-header">
        <h3>☀️ Solar Indices</h3>
        <span className="data-source">{solarIndices.source}</span>
      </div>
      ...
      <div className="card-footer">
        <div className="info-box">
          ...
          <ul>
            <li><strong>SFI:</strong> Higher values (150+) indicate better HF propagation</li>
            <li><strong>K-Index:</strong> Lower values (0-2) mean quieter, more stable conditions</li>
            <li><strong>A-Index:</strong> 24-hour average geomagnetic activity (lower is better)</li>
          </ul>
        </div>
      </div>
    </div>
...
export default SolarIndicesCard;
